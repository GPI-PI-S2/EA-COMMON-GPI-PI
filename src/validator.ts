...
class Validator {
	private static errors = {
		...
		nickname:
			'El nombre de usuario contiene caracteres no válidos (solo letras minúsculas,números y puntos)',
	};
	static rules = {
		required: (i: string, scope?: Validator.Scope) =>
			(i ? true : false) || Validator.error('required', scope),
		...
		rangeMin: (i: string, min: number, scope?: Validator.Scope) =>
			(i && Number(i) && Number(i) >= min) ||
			Validator.error('range-min', scope, min.toString()),
		rangeMax: (i: string, max: number, scope?: Validator.Scope) =>
			(i ? Number(i) && Number(i) <= max : true) ||
			Validator.error('range-max', scope, max.toString()),
		...
		phone: (i: string, scope?: Validator.Scope) =>
			(i ? /^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$/.test(i) : true) ||
			Validator.error('phone', scope),
		...
		url: (i: string, scope?: Validator.Scope) =>
			(i
				...
				: true) || Validator.error('url', scope),
		...
		nickname: (i: string, scope?: Validator.Scope) =>
			(i ? /^[a-z]([a-z0-9]*[\.]?[a-z0-9]*)*[a-z0-9]+$/.test(i) : true) ||
			Validator.error('nickname', scope),
	};
	private static addToText(base: string, ...keys: string[]): string {
		...
		base.split('_$')
			.filter((e) => e !== '')
			.map((e) => {
				const el = e.split('_')[0];

				if (!Number.isNaN(el) && keys[Number(el) - 1])
					response = (response ? response : base).replace(
						`_$${Number(el)}_`,
						keys[Number(el) - 1],
					);
				return e;
	...
	private static error(e: string, scope?: Validator.Scope, ...a: string[]): string {
		...
		return Validator.addToText((Validator as { [key: string]: any }).errors[cError], ...a);
	}
}
